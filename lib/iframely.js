(function(proxy) {
    ...
    var iframelyMeta = require('./iframely-meta'),
        whitelist = proxy.whitelist = require('./whitelist'),
        utils = require('./utils.js'),
        disposeObject = proxy.disposeObject = utils.disposeObject;

    GLOBAL.jquerySrc = fs.readFileSync("./lib/jquery.js").toString();
        ...
        async.waterfall([
            ...
            function(cb) {
                ...
                getAllDataFromPlugins(uri, {
                    ...
                    forceOembed: options.forceOembed
                }, cb);
            },
            ...
            function(data, cb) {
                ...
                var whitelistRecord = data[0].context.whitelistRecord;
                var rawMeta = data[0].context.meta;
                        ...
                        if (r.data && r.method.name in {"getLink": 1, "getLinks": 1}) {
                            ...
                            l.forEach(function(originalLink) {
                                ...
                                if (originalLink.href || originalLink._render || originalLink._reader) {
                                    ...
                                    if (meta.canonical && link.href == meta.canonical || link.href == uri) {
                                        originalLink.error = "Removed canonical link";
                                        return;
                                    }
                                    /*
                                    ...
                                    */
                                    ...
                                    if (!options.debug && link.rel.indexOf('deny') > -1) {
                                    ...
                                    if (_.intersection(["responsive", "player"], link.rel).length == 2) {
            ...
            if (error && error.error == 'invalid-content-type' && error.headers['content-type']) {

                var link = {
                    href: error.url,
                    type: error.headers['content-type'],
                    rel: [(error.headers['content-type'].indexOf('image') !== -1) ? CONFIG.R.image : CONFIG.R.file]
                ...
                if (error.headers['content-type'].indexOf('javascript') !== -1) {
                    link = {}
                }
    ...
    function getAllDataFromPlugins(uri, options, callback) {
        ...
        var domainPlugins = pluginsList.filter(function(plugin) {
            if (plugin.domain) {
                ...
                var match = _.find(plugin.re, function(re) {
                    return uri.match(re);
                });
                if (match) {
                    return true;
        ...
        async.waterfall([
            ...
            function(data, cb) {
                ...
                var context = {
                    url: uri,
                    request: request,
                    whitelistRecord: whitelist.findWhitelistRecordFor(uri) || {},
                    cb: true
                };
                    ...
                    if (data.fullResponse) {
                        ...
                        jsdom.env({
                            ...
                            features: {
                                ...
                                // Store window instance in context to close window.
                                context.$selector._window = window;
            ...
            function(cb) {
                ...
                // Free jsdom memory.
                if (initialContext.$selector) {
                    initialContext.$selector._window.close();
                }
