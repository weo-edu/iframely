(function(proxy) {
                ...
                data.forEach(function(level) {
                    level.data.forEach(function(r) {
                        if (r.data && r.method.name in {"getLink": 1, "getLinks": 1}) {
                            ...
                            l.forEach(function(link) {
                                // TODO: generalize to cycle.
                                if (PREPARE_LINK_HANDLERS['get image'].test(link)) {
                                    links.push(link);
                                }
                                if (PREPARE_LINK_HANDLERS['safe html'].test(link)) {
                                    htmls.push(link);
                                }
                                if (PREPARE_LINK_HANDLERS['render html'].test(link, r)) {
                                    renders.push(link);
                                }
                            });
                        }
                ...
                async.parallel([
                    ...
                    function loadImages(cb) {
                        ...
                        async.each(links, function(link, cb) {
                            PREPARE_LINK_HANDLERS['get image'].handle(uri, link, options, cb);
                        }, cb);
                    },

                    function generateHtmlLinks(cb) {
                        async.each(htmls, function(link, cb) {
                            PREPARE_LINK_HANDLERS['safe html'].handle(uri, link, {}, cb);
                        }, cb);
                    },

                    function generateRenderLinks(cb) {
                        async.each(renders, function(link, cb) {
                            PREPARE_LINK_HANDLERS['render html'].handle(uri, link, {}, cb);
                        }, cb);
                    }
                                ...
                                if (originalLink.href || originalLink._render || originalLink._reader) {
                                    ...
                                    if (link.href) {
                                        ...
                                        if (link.rel.indexOf('player') > -1) {
                                            ...
                                            link.href = link.href.replace(/autoplay=1/, '$1=0');
    ...
    * */
    ...
    function getAllDataFromPlugins(uri, options, callback) {
        ...
        var initialMetaRequirements = {};

        async.waterfall([

            function firstLoad(cb) {

                async.parallel({

                    contextWithoutMeta: function(cb) {

                        async.waterfall([

                            function runWave(cb) {
                                var nonMetaRequirements = _.without(DEFAULT_REQUIREMENTS, IFRAMELY_PROVIDED_REQUIREMENTS);

                                // Find requirements.
                                var methodsData = findSuitableMethods(initialPlugins, nonMetaRequirements);
                                methodsData._allRequirements.forEach(function(req) {
                                    //initialMetaRequirements[req] = true;
                                });

                                if (methodsData.length == 0) {
                                    // Skip step if nothing to call.
                                    return cb(null, null);
                                }
                                ...
                                if ('urlMatch' in nonMetaRequirements) {
                                    if (urlMatch) {
                                ...
                                runPlugins(initialPlugins, contexts, usedMethods, cb);
                            },

                            function prepareNextWave(data, cb) {
                                ...
                                // TODO: handle initialContext.title workaround.
                                var nextContext = extractCustomData({}, result);
                    ...
                    contextWithMetaLoaded: function(cb) {
                        ...
                        async.waterfall([
                            ...
                            function(cb) {
                                ...
                                if (_.intersection(IFRAMELY_PROVIDED_REQUIREMENTS, methodsData._allRequirements).length > 0) {

                                    var opts = {
                                        meta: 'meta' in initialMetaRequirements,
                                        oembed: 'oembed' in initialMetaRequirements,
                                        fullResponse: 'html' in initialMetaRequirements || '$selector' in initialMetaRequirements,
                                        disableCache: options.disableCache
                                    };
                            ...
                            function(data, cb) {
                                ...
                                if ('urlMatch' in initialMetaRequirements) {
                                    if (urlMatch) {
                                        context.urlMatch = urlMatch;
                                    } else {
                                        // TODO: validation only for domain cases.
                                ...
                                if ('meta' in initialMetaRequirements) {
                                    if (data.meta) {
                                ...
                                if ('oembed' in initialMetaRequirements) {
                                    if (data.oembed) {
                                ...
                                if ('html' in initialMetaRequirements) {
                                ...
                                if ('$selector' in initialMetaRequirements) {
                                    if (data.fullResponse) {
                                        jsdom.env({
            ...
            function nextLoads(contexts, cb) {

                initialContext = _.extend({}, contexts.contextWithoutMeta, contexts.contextWithMetaLoaded);
    ...
    * */
    ...
    var IFRAMELY_PROVIDED_REQUIREMENTS = [
        "meta",
        "oembed",
        "html",
        "$selector"
